...
## CRITICAL: Patched TinyUSB in the SDK

`/home/pi/pico/pico-sdk/lib/tinyusb` has **uncommitted local modifications** this firmware depends on: `src/class/msc/msc_device.c` is patched so `set_sense_medium_not_present()` only fires when the app hasn't already set sense data — without it, bad-sector `MEDIUM ERROR` sense reporting breaks (plus `hub.c` robustness fixes). Never run `git submodule update`, `git checkout`, or clean the SDK tree without preserving that diff.
...
- **`msc_device.c`** — SCSI READ(10)/WRITE(10) → chunked ATA ops (64 sectors/chunk, 32 KB endpoint buffer). Drive I/O overlaps USB transfers: sequential-read prefetch and write-behind staging both run from main-loop hooks (`msc_service_prefetch`/`msc_service_write_behind`) while the RP2040 USB controller streams the previous chunk from IRQ context. A failed background write flush surfaces as a deferred MEDIUM ERROR on the next WRITE/SYNCHRONIZE CACHE; anything touching the bad-sector cache uses the strict synchronous path. Also owns power-gate wake on first I/O and ATA→SCSI sense mapping (ATA `UNC` → `MEDIUM ERROR` 03/11/00). `TEST_UNIT_READY` always answers "ready" without waking the drive, so kernel polling can't defeat power gating.
...
- **`main.c`** — boot sequence: 5s pre-delay with LED wave → `tusb_init()` → power-cycle drive → SDIO init → IDENTIFY → warm-up reads. Main loop is just `tud_task()` + idle-standby check. All boot waits go through `sleep_with_usb()` so TinyUSB stays serviced — bare `sleep_ms()` during waits causes USB enumeration failures.
...
- The attached drive has a **known bad sector at LBA 1952**, deliberately kept as a test fixture — `testscript/test.sh` reads it (TEST 4/5) and expects exactly 7,862,400 total sectors. Expected UART signature: `READ: ST=0x51 ERR ERR=0x40 UNC LBA=1952`.
